...
describe("ResultStatsComponent", () => {
  ...
  const ResultStatsComponentMounted = mount(<ResultStatsComponent stringValue={stringValue} />);
  const ResultStatsComponentShallow = shallow(<ResultStatsComponent stringValue={stringValue}  />);

  it("Contains classes", () => {
    expect(ResultStatsComponentMounted.find('.stats-container').length).to.equal(1);
  });
